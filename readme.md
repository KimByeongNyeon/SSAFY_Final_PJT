...
- 메인 페이지 기능으로, 사용자에게 주요 기능들을 간략하게 제공,
- 다른 페이지로 갈 수 있는 라우터 제공 페이지

### 메인 페이지 (기능)

![](images/MainPage2.gif)
...
- 모든 페이지에서 로그인 창으로의 접근이 가능하도록 모달창으로 구현
...
- 계정 정보, 개인 정보, 주소 정보의 세 분류로 나누어서 작성
- 계정 정보
  - 아이디, 비밀번호, 비밀번호 확인
- 개인 정보
  - 닉네임, 나이, 자본금
...
- 예, 적금 전체 데이터를 조회
- 예금, 적금, 탭으로 구분
- 적금의 경우 자유적립식, 정액적립식 두 가지의 분류로 나누어져 있음

### 금융 상품 상세 페이지 (비회원)

![](images/Product2.gif)

- 비회원이라면, 좋아요 기능, 지역 은행 찾기 기능, 댓글 기능이 막혀 있음

### 금융 상품 상세 페이지 (회원)

![](images/ProductDetailLogin.gif)

- 회원의 경우, 좋아요 기능, 지역 은행 찾기 기능, 댓글 기능 모두 수행 가능

### 게시글 CRUD

![](images/게시글%20작성.gif)
![](images/게시글%20수정삭제.gif)
...
![](images/캐러셀%20환율%20바꾸기.gif)
![](images/계산기로%20환율%20바꾸기.gif)

- 캐러셀의 있는 국기를 선택하면, 환율 계산할 수 있는 모달 창을 띄워줌
- 혹은 환율 계산기의 기능을 직접 선택하여 계산 가능
- 특히 원화간 거래 뿐만 아니라 타국 통화끼리의 환율도 계산이 가능

### 암호화폐

![](images/Crypto.gif)

- 암호화폐의 현재가를 WebSocket 통신을 통해 실시간으로 변동 값 확인 가능
- 화폐 이름 클릭 시 트레이딩 뷰 라이브러리를 통해 실시간 가격 변동 현황을 1분 단위로 그려줌

### 마이페이지

![](images/MyPage.gif)

- 내 정보 수정, 내가 좋아요한 예, 적금 확인 가능, 회원 가입 시 기입한 나이대를 기반으로 맞춤 상품 추천
- 내가 쓴 글, 댓글 조회 가능
...
- 프롬프트에 자본금, 원하는 투자 기간(개월), 예/적금으로 추천 상품시 상품 추천을 받을 수 있음
- 특정 상품에 대한 상세한 정보 요구시 응답
